...
class ACEMaker(NaiveACEFlowMaker):
    ...
    def make(self, compositions: list = None, precomputed_data: pd.DataFrame = None, structures: list = None):
        if compositions is None and structures is None:
            ...
            trainer = naive_train_ACE(read_job.output, max_steps=self.max_steps, batch_size=self.batch_size, gpu_index=self.gpu_index)
            train_checker = check_training_output(trainer.output)
            active_set_flow = ActiveStructuresFlowMaker(static_maker=self.static_maker, max_structures=self.max_structures, max_points=self.max_points, gamma_max=self.gamma_max).make(compositions, prev_dir=trainer.output.dir_name)
            ...
            trainer = naive_train_ACE(read_job.output, max_steps=self.max_steps, batch_size=self.batch_size, gpu_index=self.gpu_index)
            train_checker = check_training_output(trainer.output)
            active_set_flow = ActiveStructuresFlowMaker(static_maker=self.static_maker, max_structures=self.max_structures, max_points=self.max_points, gamma_max=self.gamma_max).make(compositions, prev_dir=train_checker.output.dir_name)
